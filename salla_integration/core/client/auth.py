...
class SallaAuth:
	...
	def exchange_code_for_tokens(self, code: str) -> dict:
		...
		try:
			...
			raise SallaAuthenticationError(message=f"Failed to exchange authorization code: {e!s}")

	def refresh_access_token(self) -> dict:
		"""Refresh the access token using the refresh token."""
		if not self._refresh_token:
			raise SallaAuthenticationError(message="No refresh token available. Please re-authenticate.")
		...
		try:
			...
			raise SallaAuthenticationError(message=f"Failed to refresh access token: {e!s}")

	def _save_tokens(self, token_data: dict):
		"""Save tokens to Salla Settings and update local cache."""
		self._access_token = token_data["access_token"]
		self._refresh_token = token_data["refresh_token"]
		self._token_expires_at = add_to_date(now_datetime(), seconds=token_data["expires_in"])
