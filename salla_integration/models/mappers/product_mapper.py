...
class ProductMapper:
    ...
    @staticmethod
    def salla_to_erpnext(salla_product: Dict[str, Any], english_salla_product: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        salla_item_categories = ProductMapper._map_salla_categories(
        ...
        erpnext_item = {
            ...
            "custom_salla_categories": ProductMapper._map_salla_categories(salla_product.get("categories", [])),
    ...
    def _map_salla_categories(salla_categories: List[Dict]) -> List[Dict]:
        ...
        for i, cat in enumerate(salla_categories):
            ...
            salla_category_name = frappe.db.get_value(
                "Salla Category",
                {"salla_category_id": salla_cat_id},
                "name"
            )
            
            if salla_category_name:
                item_categories.append({
                    "salla_category": salla_category_name,
    ...
    def _get_item_salla_categories(item_data: Dict) -> List[int]:
        ...
        for cat in item_categories:
            if cat.salla_category:
                salla_cat_id = frappe.db.get_value(
                    "Salla Category",
                    cat.salla_category,
                    "salla_category_id"
                )
    ...
    @staticmethod
    def link_item_to_categories(
        item_code: str,
        salla_categories: List[Dict]
    ) -> None:
        """
        ...
        for i, cat_data in enumerate(salla_categories):
            ...
            if salla_category_name and hasattr(salla_product_doc, "salla_item_categories"):
                salla_product_doc.append("salla_item_categories", {
                    "salla_category": salla_category_name,
                    "is_primary": 1 if i == 0 else 0
                })
    ...
    @staticmethod
    def get_primary_category(item_code: str) -> Optional[str]:
        """
