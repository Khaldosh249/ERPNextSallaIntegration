...
class CategoryMapper:
	...
	def salla_to_erpnext(salla_category: dict[str, Any]) -> dict[str, Any]:
		...
		if parent_id:
			...
			parent_salla_category = frappe.db.get_value(
				"Salla Category", {"salla_category_id": str(parent_id)}, "name"
			)
	...
	def erpnext_to_salla(salla_category: Any) -> dict[str, Any]:
		...
		if parent_category:
			parent_salla_id = frappe.db.get_value("Salla Category", parent_category, "salla_category_id")
			if parent_salla_id:
		...
		while current_id:
			...
			if salla_cat.parent_salla_category:
				current_id = frappe.db.get_value(
					"Salla Category", salla_cat.parent_salla_category, "salla_category_id"
				)
			else:
				current_id = None
	...
	def find_or_create_salla_category(
		category_name: str, salla_category_id: str | None = None, parent_salla_category: str | None = None
	) -> str:
		"""
	...
	@staticmethod
	def get_category_by_salla_id(salla_category_id: str) -> str | None:
		"""
		...
		"""
		return frappe.db.get_value("Salla Category", {"salla_category_id": salla_category_id}, "name")
	...
	def get_all_children(salla_category_name: str) -> list[str]:
		...
		"""
		lft, rgt = frappe.db.get_value("Salla Category", salla_category_name, ["lft", "rgt"]) or (0, 0)

		if not lft or not rgt:
			return []

		children = frappe.db.get_all(
			"Salla Category", filters={"lft": [">", lft], "rgt": ["<", rgt]}, pluck="name"
		)
	...
	def get_ancestors(salla_category_name: str) -> list[str]:
		...
		ancestors = frappe.db.get_all(
			"Salla Category", filters={"lft": ["<", lft], "rgt": [">", rgt]}, order_by="lft asc", pluck="name"
		)
