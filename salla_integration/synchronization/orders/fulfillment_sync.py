...
def update_fulfillment_status(
	...
	try:
		...
		if response.get("success"):
			# Update local record
			salla_order = frappe.db.get_value("Salla Order", {"salla_order_id": salla_order_id}, "name")
			if salla_order:
...
def mark_order_shipped(
	sales_order_name: str, tracking_number: str | None = None, shipping_company: str | None = None
) -> dict[str, Any]:
	"""
	...
	return update_fulfillment_status(salla_order.salla_order_id, "shipped", tracking_number, shipping_company)
...
def handle_delivery_note_submit(doc, method=None):
	...
	for item in doc.items:
		if item.against_sales_order:
			...
			salla_order = frappe.db.get_value(
				"Salla Order", {"sales_order": item.against_sales_order}, "salla_order_id"
			)
