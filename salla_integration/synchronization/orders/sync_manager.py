...
from salla_integration.core.utils.helpers import (
	...
	get_order_status_after_deivery_note_submission,
	get_secondary_warehouse,
	get_taxes_from_sales_taxes_template,
)
...
class OrderSyncManager(BaseSyncManager):
	...
	def sync_to_salla(self, sales_order_name: str) -> dict[str, Any]:
		...
		return {"status": "error", "message": "Sync to Salla not implemented for orders."}
	...
	def sync_from_salla(self, order_data: dict | None = None, **kwargs) -> dict[str, Any]:
		"""
		...
		existing_salla_order = frappe.db.get_value(
			"Salla Order", {"salla_order_id": salla_order_id}, ["name", "sales_order"], as_dict=True
		)
	...
	def create_order(self, order_data: dict) -> dict[str, Any]:
		...
		customer = self._get_or_create_customer(order_data.get("customer", {}), order_data.get("options", []))
		...
		default_taxes = (
			get_taxes_from_sales_taxes_template(default_taxes_and_charges_template)
			if default_taxes_and_charges_template
			else []
		)
		...
		return {"status": "success", "sales_order": sales_order.name, "salla_order": salla_order.name}
	...
	def _get_or_create_customer(self, customer_data: dict, order_options: list[dict]) -> str:
		...
		result = sync_manager.create_or_get_customer(customer_data, order_options)
	...
	def _get_deliverable_address(self, order_data: dict) -> str | None:
		"""
		...
		for item_data in items_data:
			...
			if not item_code:
				...
				item_code = frappe.db.get_value(
					"Salla Product", {"salla_product_id": salla_product_id}, "item_code"
				)
			...
			if item_code:
				...
				stock_in_default = (
					get_item_stock_in_warehouse(item_code, default_warehouse) if default_warehouse else 0
				)
				stock_in_secondary = (
					get_item_stock_in_warehouse(item_code, secondary_warehouse) if secondary_warehouse else 0
				)
	...
	def update_order_status_when_delivery_note_created(self, sales_order_name: str) -> None:
		...
		salla_order = frappe.db.get_value(
			"Salla Order", {"sales_order": sales_order_name}, ["name", "salla_order_id"], as_dict=True
		)
		...
		salla_order_status_record = (
			frappe.get_doc("Salla Order Status", salla_order_status_name) if salla_order_status_name else None
		)

		if not salla_order_status_record:
			print("No Salla Order Status configured for Delivery Note submission.")
			return
		...
		try:
			response = self.client.update_order_status(
				salla_order_id, str(salla_order_status_record.salla_status_id)
			)
			...
			print(f"Error updating Salla order status: {e!s}")
		...
		try:
			...
			for status in statuses:
				...
				existing = frappe.db.get_value(
					"Salla Order Status", {"salla_status_id": salla_status_id}, "name"
				)
				if not existing:
			...
			frappe.db.commit()
			return {"status": "success", "total_statuses": len(statuses)}
		except Exception as e:
			print(str(e))
			return {"status": "error", "message": str(e)}
