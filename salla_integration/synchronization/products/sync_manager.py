...
from salla_integration.core.utils.helpers import (
	get_default_price_list_for_salla_discounts,
	get_price_list_for_importing_prices_from_salla,
)
...
from salla_integration.synchronization.products.payload_builder import (
	ProductPayloadBuilder,
	ProductPayloadBuilderEn,
)
...
class ProductSyncManager(BaseSyncManager):
				...
				# ? If not found, create new product
					...
					if response.get("success") and response.get("data"):
						...
						response = self.client.update_product(
							salla_product_id, payload.get("en", {}), lang="en"
						)
						# ? Upload images
						self._sync_product_images(item, salla_product_id)
						# ? Update stock
	...
	def mark_sync_status_as_not_synced_before_start(self, item_code: str, salla_product_id: str):
		...
		if item.custom_sync_name:
			salla_product.item_name_sync_status = "Not Synced"
			frappe.db.set_value("Item", item_code, {"custom_name_sync_status": "Not Synced"})

		if item.custom_sync_description:
			salla_product.description_sync_status = "Not Synced"
			frappe.db.set_value("Item", item_code, {"custom_description_sync_status": "Not Synced"})

		if item.custom_sync_weight:
			salla_product.weight_sync_status = "Not Synced"
			frappe.db.set_value("Item", item_code, {"custom_weight_sync_status": "Not Synced"})

		if item.custom_sync_price:
			salla_product.price_sync_status = "Not Synced"
			frappe.db.set_value("Item", item_code, {"custom_price_sync_status": "Not Synced"})

		if item.custom_sync_sku:
			salla_product.sku_sync_status = "Not Synced"
			item.custom_sku_sync_status = "Not Synced"
			frappe.db.set_value("Item", item_code, {"custom_sku_sync_status": "Not Synced"})

		if item.custom_sync_categories:
			salla_product.categories_sync_status = "Not Synced"
			frappe.db.set_value("Item", item_code, {"custom_categories_sync_status": "Not Synced"})
	...
	def mark_sync_status_after_finish(self, item_code: str, salla_product_id: str, success: bool):
		...
		if item.custom_sync_name:
			...
			frappe.db.set_value("Item", item_code, {"custom_name_sync_status": status})

		if item.custom_sync_description:
			salla_product.description_sync_status = status
			frappe.db.set_value("Item", item_code, {"custom_description_sync_status": status})

		if item.custom_sync_weight:
			salla_product.weight_sync_status = status
			frappe.db.set_value("Item", item_code, {"custom_weight_sync_status": status})

		if item.custom_sync_price:
			salla_product.price_sync_status = status
			frappe.db.set_value("Item", item_code, {"custom_price_sync_status": status})
		...
		if item.custom_sync_categories:
			...
			frappe.db.set_value("Item", item_code, {"custom_categories_sync_status": status})
	...
	def sync_from_salla(self, product_data_ar: dict, product_data_en: dict, **kwargs) -> dict[str, Any]:
		...
		existing_salla_product = frappe.db.get_value(
			"Salla Product", {"salla_product_id": salla_product_id}, ["name", "item_code"], as_dict=True
		)
		...
		try:
			...
			item_data = ProductMapper.salla_to_erpnext(product_data_ar, product_data_en)
			sku = item_data.get("item_code")
			salla_item_categories = item_data.pop("_salla_item_categories", [])
			item_data.pop("_salla_id", None)
			item_data.pop("_salla_quantity", 0)
			...
			if existing_salla_product:
				...
				frappe.db.set_value(
					...
					{"item_code": item_code, "sync_status": "Synced"},
				)
		...
		try:
			...
			while has_more:
				...
				if not response_in_arabic.get("success") or not response_in_english.get("success"):
					return {
						"status": "error",
						"message": response_in_arabic.get("message", "Failed to fetch products")
						+ " / "
						+ response_in_english.get("message", "Failed to fetch products"),
						"created": created,
						"linked": linked,
						"failed": failed,
					}
				...
				for product_data_ar in products_ar:
					...
					result = self.sync_from_salla(product_data_ar, product_data_en)
					total_processed += 1
					...
					if result.get("status") == "success":
						...
						failed += 1
						print(
							f"Failed to import Salla product ID: {product_data_ar.get('id')}. Error: {result.get('message')}"
						)
		...
		try:
			...
			while has_more:
				...
				for product_data in products:
					...
					# Check if Salla Product record already exists
					existing_salla_product = frappe.db.get_value(
						"Salla Product", {"salla_product_id": str(product_data.get("id"))}, "name"
					)

					# Create Salla Product record
					if not existing_salla_product:
						self._create_salla_product_record(
							item_code=sku, salla_product_id=str(product_data.get("id"))
						)
						linked += 1
					...
					if custom_open_public_page_in_salla:
						frappe.db.set_value(
							"Item", sku, "custom_open_public_page_in_salla", custom_open_public_page_in_salla
						)

					if custom_open_admin_page_in_salla:
						frappe.db.set_value(
							"Item", sku, "custom_open_admin_page_in_salla", custom_open_admin_page_in_salla
						)
			...
			return {"status": "success", "linked": linked, "total_processed": total_processed}
		except Exception as e:
			frappe.log_error(f"Error linking Salla products to Items: {e}")
			return {"status": "error", "message": str(e), "linked": linked if "linked" in dir() else 0}
	...
	def import_products_prices_from_salla(self, page: int = 1, per_page: int = 50) -> dict[str, Any]:
		...
		try:
			...
			while has_more:
				...
				for product_data in products:
					...
					item_price_name = frappe.db.get_value(
						"Item Price", {"item_code": sku, "price_list": default_price_list}, "name"
					)
					if not item_price_name:
						...
						frappe.db.set_value("Item Price", item_price_name, "price_list_rate", amount)
			...
			return {"status": "success", "updated_prices": updated_prices, "total_processed": total_processed}
		except Exception as e:
	...
	def import_single_product(self, salla_product_id: str) -> dict[str, Any]:
		...
		try:
			...
			if not response.get("success"):
				return {"status": "error", "message": response.get("message", "Product not found")}
	...
	def _link_product_categories(self, salla_product_name: str, categories: list[dict]):
		...
		for cat in categories:
			if hasattr(salla_product, "salla_item_categories"):
				salla_product.append(
					"salla_item_categories",
					{"salla_category": cat.get("salla_category"), "is_primary": cat.get("is_primary", 0)},
				)
	...
	def build_payload(self, item) -> dict[str, Any]:
		...
		return {"ar": builder.payload, "en": builder_en.payload}
	...
	def _get_salla_product_id(self, item_code: str) -> str | None:
		"""Get the Salla product ID for an item."""
		return frappe.db.get_value("Salla Product", {"item_code": item_code}, "salla_product_id")
	...
	def _update_salla_product_record(self, item_code: str, salla_product_id: str):
		...
		salla_product_name = frappe.db.get_value("Salla Product", {"item_code": item_code}, "name")

		if salla_product_name:
			frappe.db.set_value(
				"Salla Product",
				salla_product_name,
				{"salla_product_id": salla_product_id, "sync_status": "Synced"},
			)
			frappe.db.commit()
	...
	def handle_item_rename(self, doc, method, old_name, new_name):
		...
		salla_product = frappe.db.get_value(
			"Salla Product", {"item_code": new_name}, ["name", "salla_product_id"], as_dict=True
		)

		if salla_product:
			# Update item_code in Salla Product record
			frappe.db.set_value("Salla Product", salla_product.name, "item_code", new_name)
			frappe.db.commit()
			...
			if salla_product.salla_product_id:
				...
				if sync_manager.should_sync(item) and sync_manager.should_sync_sku(item):
					payload = {"sku": new_name}
					print(f"Updating SKU in Salla for Item {new_name}")
					try:
						response = sync_manager.client.update_product(salla_product.salla_product_id, payload)
						...
						frappe.log_error(
							f"Exception while updating SKU in Salla for Item {new_name}: {e!s}",
	...
	def handle_item_price_discount_change(self, doc, method):
		...
		if not salla_product_id:
			...
			if response.get("success") and response.get("data"):
				...
				# Create Salla Product record
				self._create_salla_product_record(item_code=item_code, salla_product_id=salla_product_id)
			else:
				return
		...
		try:
			response = self.client.update_product(salla_product_id, payload)
			...
			frappe.log_error(
				f"Exception while updating discount in Salla for Item {item_code}: {e!s}",
...
def link_existing_items_with_salla_products():
	...
	if result["status"] == "error":
		frappe.log_error(
			f"Failed to link Salla products to Items: {result.get('message')}", "Salla Product Link Error"
		)
	return result
...
def _sync_item_background(item_code: str):
	...
	if result["status"] == "error":
		frappe.log_error(
			f"Failed to sync item {item_code} to Salla: {result.get('message')}", "Salla Product Sync Error"
		)
