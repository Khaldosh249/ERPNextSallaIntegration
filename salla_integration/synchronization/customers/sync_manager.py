...
class CustomerSyncManager(BaseSyncManager):
	...
	def sync_from_salla(self, customer_data: dict | None = None, **kwargs) -> dict[str, Any]:
		"""
	...
	def create_or_get_customer(
		self, customer_data: dict, order_options_details: list[dict]
	) -> dict[str, Any]:
		"""
		...
		exists = frappe.db.get_value("Customer", {"customer_name": company_name or full_name}, as_dict=True)

		if exists:
			print("Customer already exists with company_name:", company_name)
			return {"status": "exists", "customer": exists.name}

		if tax_id and tax_id not in [None, "", 0, "0", 1, "1"] and not exists:
			exists = frappe.db.get_value("Customer", {"tax_id": tax_id}, as_dict=True)
			if exists:
				print("Customer already exists with tax_id:", tax_id)
				return {"status": "exists", "customer": exists.name}

		if (
			custom_commercial_register
			and custom_commercial_register not in [None, "", 0, "0", 1, "1"]
			and not exists
		):
			exists = frappe.db.get_value(
				"Customer", {"custom_commercial_register": custom_commercial_register}, as_dict=True
			)
			if exists:
	...
	def _update_customer(self, salla_customer: dict, customer_data: dict) -> dict[str, Any]:
		...
		if full_name and salla_customer.customer:
			frappe.db.set_value("Customer", salla_customer.customer, "customer_name", full_name)
	...
	def _create_contact(
		self, customer_name: str, first_name: str, last_name: str, email: str | None, mobile: str | None
	):
