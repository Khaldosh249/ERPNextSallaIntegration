...
class BaseJob:
	...
	def enqueue(self, queue: str = "default", timeout: int = 3600, **kwargs) -> str:
		"""
		...
		job = enqueue(
			method=self._execute_job, queue=queue, timeout=timeout, job_name=self.job_name, **kwargs
		)
...
def job_handler(job_type: str = "Sync", queue: str = "default", timeout: int = 3600):
	...
	def decorator(func: Callable):
		...
		def wrapper(*args, **kwargs):
			...
			f"salla_{job_type.lower()}_{func.__name__}"
				...
				frappe.get_doc(
					{
						...
						"sync_type": job_type,
						"status": "error",
						"message": f"{job_type} job failed: {e!s}",
						"details": frappe.get_traceback(),
					}
				).insert(ignore_permissions=True)
				frappe.db.commit()
